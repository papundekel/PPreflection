...
void PPreflector::consumer::HandleTranslationUnit(clang::ASTContext& context)
{
	...
	std::ofstream out_s(std::string(get_main_file_name(context)).append(".meta"));
...
std::string_view PPreflector::consumer::get_main_file_name(clang::ASTContext& context)
{
	auto& source_manager = context.getSourceManager();

	return source_manager.getFileEntryForID(source_manager.getMainFileID())->getName();
}
