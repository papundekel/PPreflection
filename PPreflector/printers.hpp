...
namespace PPreflector
{
	...
	{
		{ p(out) } -> PP::concepts::void_type;
	};

	llvm::raw_ostream& operator<<(llvm::raw_ostream& out, const printer_enabled auto& p)
	{
		p(out);
		return out;
	}
	...
	template <PP::size_t count>
	llvm::raw_ostream& operator<<(llvm::raw_ostream& out, const constant_string<count>& str)
	{
	...
	constexpr inline auto container_printer = []
