...
namespace PPreflection::detail
{
	...
	{
		static_assert(PP::concepts::derived_from<class_type, Base>);
	...
	public:
		void destroy(void* ptr) const noexcept override final
		{
			if constexpr (PP::concepts::destructible<T>)
				destroy_at(reinterpret_cast<T*>(ptr));
		}
		...
		constexpr PP::any_view_ra<const member_function::overloaded&> get_member_functions() const noexcept override final
		{
			return member_functions;
		}
		constexpr PP::any_view_ra<const static_member_function::overloaded&> get_static_member_functions() const noexcept override final
		{
			return static_member_functions;
		}

		constexpr PP::any_view_ra<const user_defined_type&> get_nested_classes() const noexcept override final
		{
