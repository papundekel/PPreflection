...
namespace PPreflection::detail
{
	...
	class basic_non_union_class_type final : public basic_class_type<T, non_union_class_type>
	{
		...
		static constexpr auto derived_class_reference_convertors = PP::tuple_map_to_array(PP::type<convertor_reference>, []
			(PP::concepts::type auto base_class_type)
			{
				return create_convertor_reference(base_class_type, PP::value<[]
					(auto&& base_r) -> auto&&
					{
						return PP::copy_cvref(PP_DECLTYPE(base_r), PP::type<T>)(base_r);
					}>);
			}, base_classes);

		static constexpr auto base_class_pointer_convertors = PP::tuple_map_to_array(PP::type<convertor_object>, []
			(PP::concepts::type auto base_class_type)
			{
				return []
					(PPreflection::dynamic_reference ref)
					{
						return ref.visit_ptr(PP::type<T>, []
							<typename U>
							(U* ptr)
							{
								return dynamic_object::create(PP::add_pointer(PP::copy_cv(PP::type<U>, PP_COPY_TYPE(base_class_type))), ptr);
							});
					};
			}, base_classes);
		...
		static constexpr auto get_conversion(const auto& convertors, const non_union_class_type& base_target) noexcept
		{
			auto classes_convertors = PP::zip_view_pack(base_classes_dynamic, convertors);
		...
		constexpr convertor_object pointer_conversion_to_base(const non_union_class_type& base_target) const noexcept override final
		{
			if (auto c = get_conversion(base_class_pointer_convertors, base_target); c)
				return c;
			else
				return [](dynamic_reference r){ return dynamic_object::create_shallow_copy(r); };
		}

		constexpr convertor_reference reference_conversion_to_base(const non_union_class_type& base_target) const noexcept override final
		{
			if (auto c = get_conversion(base_class_reference_convertors, base_target); c)
				return c;
			else
				return [](dynamic_reference r){ return r; };
		}

		constexpr convertor_reference reference_conversion_to_derived(const non_union_class_type& base_target) const noexcept override final
		{
			if (auto c = get_conversion(derived_class_reference_convertors, base_target); c)
				return c;
			else
				return [](dynamic_reference r){ return r; };
		}
