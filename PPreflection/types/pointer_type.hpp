...
PPreflection::pointer_type::make_standard_conversion_sequence_impl(
	const pointer_type& target) const noexcept
{
	...
	auto pointed_to_this_function_ptr =
		dynamic_cast<const function_type*>(&pointed_to_this.type);
	auto pointed_to_target_function_ptr =
		dynamic_cast<const function_type*>(&pointed_to_target.type);

	if (pointed_to_this_function_ptr && pointed_to_target_function_ptr)
	{
		auto convertible_rank =
			pointed_to_this_function_ptr->convertible_function_type(
				*pointed_to_target_function_ptr);
	...
	else if (pointed_to_target.cv >= pointed_to_this.cv)
	{
		...
		if (derived)
		{
			...
			sequence.set_promotion_conversion(
				pointed_to_this_class_ptr->pointer_conversion_to_base(
					*pointed_to_target_class_ptr));
	...
	const non_array_object_type& target) const noexcept
{
	...
	if (const auto* target_pointer_ptr =
			dynamic_cast<const pointer_type*>(&target);
		target_pointer_ptr)
	{
