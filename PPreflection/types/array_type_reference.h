...
namespace PPreflection
{
	namespace detail
	{
		using array_type_reference_helper = PP::variant<const known_bound_array_type&, const unknown_bound_array_type&>;
	}
	...
	private:
		...
		{
			using RET = PP::variant<const unknown_bound_array_type&, const complete_object_type&>;

			return PP::visit(PP::overloaded
			(
				[](const unknown_bound_array_type& t){ return RET(PP::placeholder, t); },
				[](const   known_bound_array_type& t){ return RET(PP::placeholder, t); }
			), *this);
		}
	...
	public:
		...
		{
			return PP::visit([name](const auto& array_type) { return array_type.has_name(name);	}, *this);
		}

		constexpr void print_name_prefix(PP::simple_ostream& out) const noexcept override final
		{
			PP::visit([&out](const auto& array_type) { array_type.print_name_prefix(out); }, *this);
		}

		constexpr void print_name_suffix(PP::simple_ostream& out) const noexcept override final
		{
			PP::visit([&out](const auto& array_type) { array_type.print_name_suffix(out); }, *this);
		}

		constexpr cv_type<complete_object_type> remove_extent() const noexcept override final
		{
			return PP::visit([](const auto& array_type)	{ return array_type.remove_extent(); }, *this);
		}

		constexpr convertor_object array_to_pointer_conversion() const noexcept override final
		{
			return PP::visit([](const auto& array_type) { return array_type.array_to_pointer_conversion(); }, *this);
		}

		constexpr bool operator==(const type& other) const noexcept override final
		{
			return PP::visit([&other](const auto& array_type) { return array_type == other; }, *this);
		}

		constexpr const pointer_type& get_pointer_to_element() const noexcept override final
		{
			return PP::visit([](const auto& array_type) -> auto& { return array_type.get_pointer_to_element(); }, *this);
		}
	};
}
