...
///
/// @brief Represents a cv qualified type pointer.
///
...
///
template <typename Type>
class cv_type : public type
{
	friend class dynamic_object;
	friend class known_bound_array_type;
	friend class pointer_type;
	friend class reference_type;
	friend class unknown_bound_array_type;
...
public:
	...
	constexpr PP::variant<const reference_type&, const pointable_type&>
		cast_down(
			PP::overload_tag<PPreflection::type>) const noexcept override final
	{
...
private:
	...
	constexpr auto cast(PP::concepts::type auto t) const
	{
		return PP::Template<cv_type>(t)(
			PP::dynamic__cast(t + PP::add_const_tag + PP::add_lvalue_tag, type),
			cv);
	}
};
template <typename Type>
