...
namespace PPreflection
{
	...
	{
		template <typename T>
		using arithmetic_type_strong_base = PP_GET_TYPE(PP::conditional(PP::value<PP::concepts::integral<T>>, PP::type<integral_type>, PP::type<arithmetic_type>));
	}

	template <typename T>
	class arithmetic_type_strong : public
		detail::basic_non_array_object_type<T, 
		detail::basic_named_descriptor<T,
		detail::arithmetic_type_strong_base<T>>>
	{
		static_assert(PP::concepts::non_void_fundamental<T> && PP::concepts::different<T, decltype(nullptr)>);

	public:
		constexpr const arithmetic_type& promoted_type() const noexcept override final
		{
