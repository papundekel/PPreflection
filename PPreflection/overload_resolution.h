...
{
	PP::vector<detail::implicit_conversion_sequence> conversion_sequences;
...
public:
	...
	detail::implicit_conversion_sequence get_first_sequence() const noexcept
	{
		return /*PP::move*/ (detail::implicit_conversion_sequence &&)(
			conversion_sequences[0]);
	}

	void make_conversion_sequences(PP::concepts::view auto&& argument_types,
	                               bool can_use_user_defined);

	bool has_invalid_conversion() const
	{
		for (auto& s : conversion_sequences)
			if (s.get_type() ==
			    detail::implicit_conversion_sequence::type::invalid)
				return true;

		return false;
	}

	bool operator>(const viable_function& other) const noexcept
	{
...
{
	detail::standard_conversion_sequence return_value_sequence;

public:
	explicit viable_function_with_return_sequence(const Function& f)
		: viable_function<Function>(f)
		, return_value_sequence(
			  detail::standard_conversion_sequence::create_invalid())
	{}

	void set_return_value_sequence(
		detail::standard_conversion_sequence sequence) noexcept
	{
		return_value_sequence = PP::move(sequence);
	}

	auto make_conversion_sequence() const noexcept
	{
		return this->get_first_sequence()
		    .with_user_defined_conversion(this->get_function())
		    .with_second_standard_conversion(
				/*PP::move*/ (detail::standard_conversion_sequence &&)(
					return_value_sequence));
	}
...
{
	friend candidate_functions;
	friend class initialization;
	friend viable_functions;

	static void pick_viable_candidates(PP::concepts::view auto&& candidates,
	...
	static auto pick_best_viable_function(
	...
	static auto resolve(PP::concepts::view auto&& candidates,
	...
	static auto resolve(PP::concepts::view auto&& candidates,
	                    PP::concepts::view auto&& argument_types)
	{
		return resolve(PP_F(candidates), PP_F(argument_types), nullptr, true);
	}
};
}
