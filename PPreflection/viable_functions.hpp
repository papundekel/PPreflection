...
namespace PPreflection
{
			...
			: arg_function_pairs()
		{
			for (auto&& argument_list : PP_FORWARD(argument_lists))
			{
				arg_function_pairs.push_back(PP::forward_as_tuple(
					argument_list_type::create_copy_view(PP_FORWARD(argument_list)),
					overload_resolution(PP_FORWARD(candidates), PP_FORWARD(argument_list))[PP::value_0]));
			}
		}
	...
	private:
		dynamic_variable invoke_impl(PP::concepts::view auto&& arguments) const
		{
			...
			{
				if (PP::view_equal(arg_types | PP::transform(PP::unref), invoked_arg_types))
				{
					if (f)
