...
namespace PPreflection
{
	...
	class type_info_map
	{
		using pair = PP::tuple<std::type_index, PP::reference_wrapper<const non_union_class_type&>>;
	...
	public:
		...
		explicit type_info_map(PP::type_tuple_t<T...>)
			: key_values{pair(PP::placeholder, typeid(T), reflect(PP::type<T>))...}
		{
			std::sort(key_values, key_values + Count, []
		...
		{
			auto i = std::lower_bound(key_values, key_values + Count, type, []
