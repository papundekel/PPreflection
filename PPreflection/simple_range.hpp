...
template <typename T>
constexpr auto simple_range<T>::begin() noexcept
{
	return begin_;
}

template <typename T>
constexpr auto simple_range<T>::end() noexcept
{
	return end_;
}

template <typename T>
constexpr auto simple_range<T>::count() noexcept
{
	return end() - begin();
}

template <typename T>
constexpr auto& simple_range<T>::operator[](std::size_t index) noexcept
{
	return begin()[index];
}
